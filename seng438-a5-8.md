...
When assessing system reliability, both Reliability Growth Analysis and RDC are based partly on a target failure rate λ<sub>F</sub> (or MTTF). Given an ideal "no risk" scenario, it is only acceptable if the system passes its target MTTF without failure. However, in the real world where it is difficult to achieve a "no risk" condition, risk parameters are introduced and can be applied to an RDC as an efficient way of verifying if a given target failure rate is met. In an assessment of the CDS dataset with risk parameters kept constant at default (see below), our minimum MTTF was 0.69. An alternative approach to meeting assessment criteria in Reliability Growth is through the use of SRTAT or C-SFRAT to analyse the trend of changing λ. Using C-SFRAT, the top 2 models we discovered for our dataset were IFRGSB and TL which both had an MTTF of 0.117.

# Advantages and Disadvantages of Reliability Growth

The advantage of Reliability Growth is that it shows the reliability of the SUT overtime, we can identify and rectify the defects and weaknesses of the system based on the changes on the graph. 
The disadvantages of Reliability Growth are that it requires significant time as we require more failure data to analyze the reliability of the SUT.
